...
try {
    if (tetrisPlaygroundTarget) drawTetrisPlayground(10, 20, tetrisPlaygroundTarget);
    else throw new Error("Target element not found");
} catch (e) {
    console.log(e.message);
}
const shapeKeys = Object.keys((0, _shapes.shapes));
const shapeKeyIndex = Math.floor(Math.random() * shapeKeys.length);
const shapeKey = shapeKeys[shapeKeyIndex];
const currentShape = (0, _shapes.shapes)[shapeKey];
function renderShape() {
    ...
    for(let rowIndex = 0; rowIndex < rowsToColor; rowIndex++){
        ...
        for(let cellIndex = 0; cellIndex < cellsToColor; cellIndex++){
            ...
            if (cell && currentShape.shape[rowIndex][cellIndex]) cell.style.backgroundColor = currentShape.color;
        }
    }
}
function rotateShape(shape) {
    ...
    const rotatedShape = [];
    for(let rowsCount = 0; rowsCount < shape[0].length; rowsCount++)rotatedShape.push([]);
    for(let i = shape.length - 1, k = 0; i >= 0; i--, k++)for(let j = 0; j < shape[0].length; j++)rotatedShape[j][k] = shape[i][j];
    return rotatedShape;
}
function removePreviousShape() {
    ...
    for(let rowIndex = 0; rowIndex < rowsToColor; rowIndex++){
        ...
        for(let cellIndex = 0; cellIndex < cellsToColor; cellIndex++){
            ...
            if (cell && currentShape.shape[rowIndex][cellIndex]) cell.style.backgroundColor = "";
        }
    }
}
renderShape();
...
},{"./shapes":"1MopT"}],"1MopT":[function(require,module,exports) {
